...
1. В логической рабочей области **Cisco Packet Tracer** была подготовлена общая топология сети организации. В схему были включены маршрутизаторы, коммутаторы, серверы, ПК, ноутбуки и промежуточные устройства сети провайдера.
   ...
   ![Общая топология сети](01.png){ #fig:001 width=80% }

2. На коммутаторе провайдера **provider-babdullakhi-sw-1** были настроены trunk-порты для передачи трафика нескольких VLAN между сетевыми устройствами.
   ...
   ![Настройка VLAN и trunk-портов на коммутаторе провайдера](02.png){ #fig:002 width=80% }

3. На маршрутизаторе центральной площадки **msk-donskaya-babdullakhi-gw-1** были созданы подинтерфейсы для подключения к сетям через провайдера.
   ...
   ![Настройка подинтерфейсов на маршрутизаторе центральной площадки](03.png){ #fig:003 width=80% }

4. На маршрутизаторе **msk-donskaya-babdullakhi-gw-1** были добавлены статические маршруты до удалённых сетей.
   ...
   ![Настройка статических маршрутов и ACL на маршрутизаторе центральной площадки](04.png){ #fig:004 width=80% }

5. На маршрутизаторе основного здания в 42-м квартале **msk-q42-babdullakhi-gw-1** были настроены подинтерфейсы для локальных VLAN.
   ...
   ![Настройка маршрутизатора сети 42-го квартала](05.png){ #fig:005 width=80% }

6. На коммутаторе **msk-q42-babdullakhi-sw-1** была выполнена настройка подключения к маршрутизатору и пользовательскому ПК.
   ...
   ![Настройка коммутатора сети 42-го квартала](06.png){ #fig:006 width=80% }

7. На оконечном устройстве **msk-q42-babdullakhi-pc-1** была выполнена статическая настройка сетевых параметров.
   ...
   Указанный шлюз соответствует адресу подинтерфейса маршрутизатора **msk-q42-babdullakhi-gw-1** в пользовательской сети VLAN 201. Это позволяет ПК передавать трафик в другие сети через маршрутизатор.

   ![Настройка IP-адреса на ПК сети 42-го квартала](07.png){ #fig:007 width=80% }
   
   8. На устройстве **msk-hostel-babdullakhi-gw-1** была выполнена настройка связи между сетью управления 42-го квартала и локальной сетью общежития.
   ...
   ![Настройка шлюза сети общежития](08.png){ #fig:008 width=80% }

9. На коммутаторе **msk-hostel-babdullakhi-sw-1** была выполнена настройка подключения к шлюзу сети общежития и пользовательскому ПК.
   ...
   ![Настройка коммутатора сети общежития](09.png){ #fig:009 width=80% }

10. На оконечном устройстве **msk-hostel-babdullakhi-pc-1** была выполнена статическая настройка сетевых параметров.
    ...
    Адрес шлюза **10.129.128.1** соответствует виртуальному интерфейсу **Vlan301** на устройстве **msk-hostel-babdullakhi-gw-1**. Благодаря этому компьютер сети общежития может передавать трафик в другие сети через настроенный шлюз.

    ![Настройка IP-адреса на ПК сети общежития](10.png){ #fig:010 width=80% }

11. На маршрутизаторе филиала в Сочи **sch-sochi-babdullakhi-gw-1** была выполнена настройка подключения к сети провайдера и локальным сетям филиала.
    ...
    ![Настройка маршрутизатора филиала в Сочи](11.png){ #fig:011 width=80% }

12. На коммутаторе **sch-sochi-babdullakhi-sw-1** была выполнена настройка trunk-соединений и пользовательской VLAN.
    ...
    ![Настройка коммутатора филиала в Сочи](12.png){ #fig:012 width=80% }

13. На оконечном устройстве **sch-sochi-babdullakhi-pc-1** была выполнена статическая настройка сетевых параметров.
    ...
    Шлюз **10.130.0.1** соответствует подинтерфейсу **FastEthernet0/0.401** маршрутизатора **sch-sochi-babdullakhi-gw-1**. Это позволяет компьютеру филиала обращаться к удалённым сетям через маршрутизатор Сочи и сеть провайдера.

    ![Настройка IP-адреса на ПК филиала в Сочи](13.png){ #fig:013 width=80% }
	
14. Была выполнена проверка прохождения маршрутов с административного компьютера **babdullakhi-admin-donskaya-1** до удалённых узлов.
    ...
    ![Проверка маршрутов командой tracert](14.png){ #fig:014 width=80% }

15. На маршрутизаторе центральной площадки **msk-donskaya-babdullakhi-gw-1** была проверена таблица маршрутизации командой **show ip route**.
    ...
    ![Таблица маршрутизации маршрутизатора центральной площадки](15.png){ #fig:015 width=80% }

16. На маршрутизаторе **msk-q42-babdullakhi-gw-1** была проверена таблица маршрутизации.
    ...
    ![Таблица маршрутизации маршрутизатора 42-го квартала](16.png){ #fig:016 width=80% }

17. На устройстве **msk-hostel-babdullakhi-gw-1** была проверена таблица маршрутизации.
    ...
    ![Таблица маршрутизации шлюза сети общежития](17.png){ #fig:017 width=80% }

18. На маршрутизаторе филиала в Сочи **sch-sochi-babdullakhi-gw-1** была проверена таблица маршрутизации.
    ...
    ![Таблица маршрутизации маршрутизатора филиала в Сочи](18.png){ #fig:018 width=80% }
